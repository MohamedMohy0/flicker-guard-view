...
function Dashboard() {
  ...
  const buyCredits = () => {
    const message = encodeURIComponent(
      "hello i want to buy some credits to safe my files"
    );

    window.open(
      `https://wa.me/201110037173?text=${message}`,
      "_blank"
    );
  };
  ...
  const onFile = async (
    ...
    if (!profile || profile.credits <= 0) {
      toast.error(
        "You're out of credits. Upgrade to add more."
      );
      return;
    }
    ...
    try {
      ...
      const pages = await processPdf(file, (c, t) =>
        setProgress(`Securing page ${c}/${t}…`)
      );
      ...
      for (let i = 0; i < pages.length; i++) {
        setProgress(
          `Uploading page ${i + 1}/${pages.length}…`
        );
        ...
        const aUrl = supabase.storage
          .from("pdf-pages")
          .getPublicUrl(`${base}_a.png`).data.publicUrl;

        const bUrl = supabase.storage
          .from("pdf-pages")
          .getPublicUrl(`${base}_b.png`).data.publicUrl;
    ...
    <main className="min-h-screen">
      ...
      <section className="mx-auto max-w-5xl px-6 py-10">
        ...
        <p className="mt-1 text-muted-foreground">
          Upload a PDF get a private link that fights
          cameras.
        </p>
        ...
        <div className="mt-8 rounded-2xl border border-dashed border-border bg-card/40 p-10 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Each upload costs 1 credit. Processed
