...
function Viewer() {
  ...
  useEffect(() => {
    if (!pageData || !images || !canvasRef.current) {
      return;
    }
    ...
    const draw = () => {
      ...
      if (ready && !blocked) {
        ...
        parity = (frameCounter >> 1) % 2;

        if (Math.random() > 0.94) {
          parity ^= 1;
        }

        temporalShift = Math.sin(frame * 0.07) * 1.2;
        ...
        ctx.drawImage(parity ? images.b : images.a, temporalShift, 0);
        ...
        for (let y = 0; y < pageData.h; y += 24) {
          const shift = Math.sin((y + frame) * 0.05) * 2;
        ...
        const flashY = (frame * 5) % pageData.h;
        ...
        ctx.fillRect(0, flashY, pageData.w, 180);
        ...
        for (let i = 0; i < 300; i++) {
          const x = Math.random() * pageData.w;
        ...
        const vShift = Math.sin(frame * 0.13) * 1.5;
        ...
        for (let y = 0; y < pageData.h; y += 3) {
          const wave = Math.sin(y * 0.08 + frame * 0.04) * 2;

          ctx.fillStyle = y % 2 === 0
            ? "rgba(255,255,255,0.05)"
            : "rgba(0,0,0,0.03)";

          ctx.fillRect(wave, y, pageData.w, 1);
        }
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (wmRef.current) {
        ...
        wmRef.current.style.transform = `translate(-50%, -50%) rotate(${Math.sin(t) * 8}deg)`;
      }
    ...
    >
      ...
      <div className="flex justify-center py-6">
        ...
        >
          ...
          <div ref={flickerRef} className="flicker-layer" />
          ...
          {blocked && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black text-center text-destructive">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Bring this window back to focus to resume.
              </p>
            </div>
          )}
        </div>
